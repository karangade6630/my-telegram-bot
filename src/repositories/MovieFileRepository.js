...
export class MovieFileRepository extends BaseRepository {
	...
	async unlink(movieId, fileId) {
		await this.run('DELETE FROM movie_files WHERE movie_id = ? AND file_id = ?', [movieId, fileId]);
	}
	...
	async getFileIds(movieId) {
		const rows = await this.all('SELECT file_id FROM movie_files WHERE movie_id = ? ORDER BY sort_order ASC', [movieId]);
	...
	async exists(movieId, fileId) {
		return super.exists('movie_files', 'movie_id = ? AND file_id = ?', [movieId, fileId]);
	}
