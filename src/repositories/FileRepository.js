...
export class FileRepository extends BaseRepository {
	...
	async findByUniqueId(uniqueId) {
		const row = await this.first('SELECT * FROM files WHERE unique_id = ?', [uniqueId]);
		return row ? FileModel.fromRow(row) : null;
	}
	...
	async findByFileId(telegramFileId) {
		const row = await this.first('SELECT * FROM files WHERE telegram_file_id = ?', [telegramFileId]);
		return row ? FileModel.fromRow(row) : null;
	}
	...
	async insert(data) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			[
				...
				data.language ?? null,
				data.audio_tracks ?? null,
				data.subtitle ?? null,
				data.codec ?? null,
				data.is_hevc ? 1 : 0,
				data.is_hdr ? 1 : 0,
				data.is_dual_audio ? 1 : 0,
				data.season ?? null,
				data.episode ?? null,
				data.size ?? null,
