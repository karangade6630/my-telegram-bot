import { normalizeMovieTitle, validateSearchQuery } from '../utils/validation.js';
...
export class MovieSearchService {
	...
	async search(query) {
		...
		if (validateSearchQuery(normalized)) {
			return [];
		}
      ...
      LIMIT 10
    `;
		const exactRows = await this.repository.execute(sqlExact, [normalized]);
		if (exactRows.results && exactRows.results.length > 0) {
			return exactRows.results;
		}
